...
__kernel void render_kernel(__global float* output, int width, int height, int rendermode)
{
	...
	if (t > 1e19 && rendermode != 1){
		//output[work_item_id] = fy * 0.1f;
		//output[work_item_id + 1] = fy * 0.3f;
		//output[work_item_id + 2] = 0.3f;

		output[(work_item_id * 3) + 0] =  fy * 0.1f;
        output[(work_item_id * 3) + 1] = fy * 0.3f;
        output[(work_item_id * 3) + 2] = 0.3f;
	...
	float3 result = sphere1.color * cosine_factor; /* with cosine weighted colour */
	output[(work_item_id * 3) + 0] = result.x;
	output[(work_item_id * 3) + 1] = result.y;
	output[(work_item_id * 3) + 2] = result.z;
	//printf("%d, %d, %f, %f, %f\n", x_coord, y_coord, output[work_item_id][0], output[work_item_id][1], output[work_item_id][2]);
}
